...
def to_categorical(img, n_classes=3):  #üç katmanı preslemeye npy fotoyu çekmek için ilk adım
    stack = []
...
if __name__ == "__main__":
    ...
    DATASET_LOBE_MASK_PATH           = constants.DATASET_LOBE_MASK_PATH             # get dataset mask path from constants
   ...
   # DATASET_ORIGINAL_JPG_EXT    = constants.DATASET_ORIGINAL_JPG_EXT      # get dataset jpg extension from constants
    ...
    mask_image_paths           = sorted(glob.glob(DATASET_LOBE_MASK_PATH+'*'+DATASET_MASK_EXT))                       # get mask image paths
    ...
    for i,img_path in enumerate(original_image_dicom_paths):           # enumarate original image paths
        try:    
            _,img_name = os.path.split(img_path)                           # get image name
            img_name   = img_name.replace(".jpg", "")                      # replace extension
            temp_mask  = imread(mask_image_paths[i],cv2.IMREAD_GRAYSCALE)[:,:,-1]  # load for png extension
            #temp_mask  = temp_mask.astype(np.bool_)                        # convert mask to boolean
            
            # temp_original_img,top_left, bottom_right  = data_generator(img_path,'original',"","")  # get cropped original image and cell corners
            temp_original_img                           = data_generator(img_path,'original',"","")  # get cropped original image and cell corners
            ...
            # temp_mask         = temp_mask[top_left[0]:bottom_right[0],top_left[1]:bottom_right[1]]             # crop the mask image by various cell size
            temp_mask          = resize(temp_mask, (256, 256), mode='constant', preserve_range=True)            # Resize images to 256x256
            ...
            if (1 - (counts[0]/counts.sum())) > 0:           # at least 5% useful area with labels that are not 0
                ...
                mask_stack = to_categorical(temp_mask, n_classes)
